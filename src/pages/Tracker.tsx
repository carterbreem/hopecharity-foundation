...
export default function Tracker() {
  ...
  // Has the user triggered a Track search yet? (Option B behaviour)
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      .from('applications')
      .select('*')
      .eq('reference_number', searchRef.trim().toUpperCase())
      .maybeSingle();
    ...
    // If the searched app belongs to this user, also load its messages
    if (user && (data as Application).user_id === user.id) {
      const { data: msgData } = await supabase
        .from('application_messages')
        .select('*')
        .eq('application_id', (data as Application).id)
        .order('created_at', { ascending: true });
      if (msgData) {
        setMessagesMap((prev) => ({
          ...prev,
          [(data as Application).id]: msgData as ApplicationMessage[],
        }));
      }
    }
  };
      ...
      <section className="section-padding bg-white">
        <div className="container-max">
          ...
          <div className="max-w-2xl">
            ...
            {/* Only show result card AFTER Track is clicked */}
            {hasTracked && searchResult && (
              <>
                {/* Admin message (if any) — displayed after welcome, before application info */}
                {(() => {
          ...
          {/* My Applications — only shown AFTER the user has clicked Track (Option B) */}
          {hasTracked && (
            <div className="mt-10">
